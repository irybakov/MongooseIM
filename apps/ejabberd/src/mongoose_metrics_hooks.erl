...
-spec sm_register_connection_hook(mongoose_acc:t(), tuple(), ejabberd:jid(), term()
                                 ) -> mongoose_acc:t().
...
-spec sm_remove_connection_hook(mongoose_acc:t(), tuple(), ejabberd:jid(),
...
-spec auth_failed(mongoose_acc:t(), binary(), binary()) -> mongoose_acc:t().
auth_failed(Acc, _,Server) ->
    mongoose_metrics:update(Server, sessionAuthFails, 1),
    Acc.

-spec user_send_packet(mongoose_acc:t(), ejabberd:jid(), tuple(), tuple()
                      ) -> mongoose_acc:t().
...
-spec user_receive_packet(mongoose_acc:t(), ejabberd:jid(), tuple(), tuple(), tuple()) ->
    mongoose_acc:t().
...
-spec xmpp_bounce_message(Acc :: mongoose_acc:t(), Server :: ejabberd:server(),
                          tuple()) -> mongoose_acc:t().
xmpp_bounce_message(Acc, Server, _) ->
    mongoose_metrics:update(Server, xmppMessageBounced, 1),
    Acc.

-spec xmpp_stanza_dropped(mongoose_acc:t(), ejabberd:jid(), tuple(), tuple()) ->
    mongoose_acc:t().
xmpp_stanza_dropped(Acc, #jid{server = Server} ,_,_) ->
    mongoose_metrics:update(Server, xmppStanzaDropped, 1),
    Acc.

-spec xmpp_send_element(Acc :: mongoose_acc:t(), Server :: ejabberd:server(),
...
-spec roster_get(mongoose_acc:t(), {_, ejabberd:server()}) -> mongoose_acc:t().
roster_get(Acc, {_, Server}) ->
    mongoose_metrics:update(Server, modRosterGets, 1),
    Acc.

-spec roster_set(Acc :: mongoose_acc:t(), JID :: ejabberd:jid(), tuple(), tuple()) ->
    mongoose_acc:t().
roster_set(Acc, #jid{server = Server},_,_) ->
    mongoose_metrics:update(Server, modRosterSets, 1),
    Acc.

-spec roster_in_subscription(mongoose_acc:t(), binary(), binary(), tuple(), atom(), term()) ->
    mongoose_acc:t().
...
-spec roster_push(mongoose_acc:t(), ejabberd:jid(), term()) -> mongoose_acc:t().
...
-spec register_user(mongoose_acc:t(), binary(), ejabberd:server()) -> mongoose_acc:t().
register_user(Acc, _,Server) ->
    mongoose_metrics:update(Server, modRegisterCount, 1),
    Acc.

-spec remove_user(mongoose_acc:t(), binary(), ejabberd:server()) -> mongoose_acc:t().
...
-spec privacy_iq_get(mongoose_acc:t(), ejabberd:jid(), ejabberd:jid(), term(), term()) ->
    mongoose_acc:t().
...
-spec privacy_iq_set(Acc :: mongoose_acc:t(),
                     From :: ejabberd:jid(),
                     _To :: ejabberd:jid(),
                     _IQ :: ejabberd:iq()) -> mongoose_acc:t().
...
-spec privacy_check_packet(Acc :: allow | deny | block | mongoose_acc:t(),
                          binary(),
                          Server :: ejabberd:server(),
                          term(), term(), term()) -> allow | deny | block | mongoose_acc:t().
...
-spec mam_remove_archive(Acc :: mongoose_acc:t(),
                         Host :: ejabberd:server(),
                         _ArcID :: mod_mam:archive_id(),
                         _ArcJID :: ejabberd:jid()) -> mongoose_acc:t().
...
-spec mam_flush_messages(Acc :: mongoose_acc:t(),
                         Host :: ejabberd:server(),
                         MessageCount :: integer()) -> mongoose_acc:t().
mam_flush_messages(Acc, Host, MessageCount) ->
    mongoose_metrics:update(Host, modMamFlushed, MessageCount),
    Acc.

-spec mam_drop_message(Acc :: mongoose_acc:t(), Host :: ejabberd:server()) ->
    mongoose_acc:t().
mam_drop_message(Acc, Host) ->
    mongoose_metrics:update(Host, modMamDropped, 1),
    Acc.

-spec mam_drop_iq(Acc :: mongoose_acc:t(), Host :: ejabberd:server(), _To :: ejabberd:jid(),
    _IQ :: ejabberd:iq(), _Action :: any(), _Reason :: any()) -> mongoose_acc:t().
mam_drop_iq(Acc, Host, _To, _IQ, _Action, _Reason) ->
    mongoose_metrics:update(Host, modMamDroppedIQ, 1),
    Acc.

-spec mam_drop_messages(Acc :: mongoose_acc:t(),
                        Host :: ejabberd:server(),
                        Count :: integer()) -> mongoose_acc:t().
